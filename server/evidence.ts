...
export function assessObservation(observation: Observation, context: ProjectContext): ObservationAssessment {
  ...
  if (!target || !operationValid(observation)) {
    const details = [...observation.operation.reasons, ...observation.targetHealth.filter((entry) => !entry.healthy).map((entry) => entry.detail || `${entry.targetId} reported unhealthy`), ...observation.measurements.filter((entry) => entry.quality !== "valid").map((entry) => `${entry.channel} was marked invalid`)].filter(Boolean);
    return { observationId: observation.id, planId: observation.planId, valid: false, classification: "INVALID", summary: details.length ? `HC-SR04 operation rejected: ${details.join("; ")}.` : "The operation or measurement failed validation and cannot support a diagnosis.", limitations: observation.limitations };
  }
  if (target.type === "hc_sr04") {
    ...
    let summary = "HC-SR04 timing behavior satisfies the declared profile bounds.";
    if (!withinBounds) {
      const reasons: string[] = [];
      if (distance !== undefined && target.expected.distanceCm && !inDistanceRange) reasons.push(`distance ${distance.toFixed(1)} cm is outside ${target.expected.distanceCm[0]}–${target.expected.distanceCm[1]} cm`);
      if (timeoutRate !== undefined && timeoutRate > target.expected.maximumTimeoutRate) reasons.push(`timeout rate ${(timeoutRate * 100).toFixed(1)}% exceeds ${(target.expected.maximumTimeoutRate * 100).toFixed(1)}%`);
      if (deviation !== undefined && deviation > target.expected.maximumStddevCm) reasons.push(`standard deviation ${deviation.toFixed(2)} cm exceeds ${target.expected.maximumStddevCm} cm`);
      if (progression === false) reasons.push("declared target progression was not observed");
      summary = `HC-SR04 ${reasons.length ? reasons.join("; ") : "did not return a complete valid measurement"}.`;
    }
    return { observationId: observation.id, planId: observation.planId, valid: true, classification: withinBounds ? "VALID" : "MALFORMED", summary, limitations: observation.limitations };
  }
