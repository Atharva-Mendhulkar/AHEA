...
export const optionalProjectContexts: Record<"hc_sr04" | "mpu6050" | "dht11", ProjectContext> = {
  hc_sr04: projectContextSchema.parse({ schemaVersion: 2, project: { id: "hc-sr04-demo", name: "HC-SR04 diagnostics", goal: "Characterize registered trigger/echo behavior without overstating distance accuracy." }, profile: { kind: "hc_sr04", moduleId: "optional.hc-sr04.v1" }, hardwareProfileId: "esp32-devkit-hc-sr04-reviewed-v1", primaryTargetId: "distance-sensor", targets: [{ id: "distance-sensor", label: "HC-SR04", type: "hc_sr04", bindingIds: ["hc_trigger", "hc_echo_protected"], echoProtection: { reviewed: true, upperOhms: 8200, lowerOhms: 10000 }, expected: { distanceCm: [2, 400], maximumStddevCm: 2, maximumTimeoutRate: .05 }, limitations: ["Geometry, alignment, and speed of sound affect the estimate."] }], allowedPlanIds: builtInModules.hc_sr04.planIds, procedures: { setupChecklist: ["Connect HC-SR04 Trigger to GPIO18, protected Echo to GPIO19, and share ground; use the reviewed Echo level interface."], interventionSafety: ["Power down before rewiring."], reference: { kind: "baseline_characterization", description: "No independent distance reference is declared." } }, constraints: commonConstraints }),
