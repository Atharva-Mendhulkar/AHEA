...
export const optionalProjectContexts: Record<"hc_sr04" | "mpu6050" | "dht11", ProjectContext> = {
  hc_sr04: projectContextSchema.parse({ schemaVersion: 2, project: { id: "hc-sr04-demo", name: "HC-SR04 diagnostics", goal: "Characterize registered trigger/echo behavior without overstating distance accuracy." }, profile: { kind: "hc_sr04", moduleId: "optional.hc-sr04.v1" }, hardwareProfileId: "esp32s3-hc-sr04-safe-disabled-v1", primaryTargetId: "distance-sensor", targets: [{ id: "distance-sensor", label: "HC-SR04", type: "hc_sr04", bindingIds: ["hc_trigger", "hc_echo_protected"], echoProtection: { reviewed: false, upperOhms: 8200, lowerOhms: 10000 }, expected: { distanceCm: [2, 400], maximumStddevCm: 2, maximumTimeoutRate: .05 }, limitations: ["Geometry, alignment, and speed of sound affect the estimate."] }], allowedPlanIds: builtInModules.hc_sr04.planIds, procedures: { setupChecklist: ["Review the 8.2 kΩ/10 kΩ Echo divider before connection."], interventionSafety: ["Power down before rewiring."], reference: { kind: "baseline_characterization", description: "No independent distance reference is declared." } }, constraints: commonConstraints }),
  mpu6050: projectContextSchema.parse({ schemaVersion: 2, project: { id: "mpu6050-demo", name: "MPU6050 diagnostics", goal: "Characterize identity, stationary behavior, drift, and motion response." }, profile: { kind: "mpu6050", moduleId: "optional.mpu6050.v1" }, hardwareProfileId: "esp32s3-mpu6050-safe-disabled-v1", primaryTargetId: "imu", targets: [{ id: "imu", label: "MPU6050", type: "mpu6050", bindingIds: ["i2c_sda", "i2c_scl"], i2c: { address: "0x68", pullupVolts: 3.3, reviewed: false }, expected: { maximumStationaryNoiseG: .08, maximumInvalidSampleRate: .05 }, limitations: ["Motion is observed at the sensor only."] }], allowedPlanIds: builtInModules.mpu6050.planIds, procedures: { setupChecklist: ["Verify SDA and SCL pull-ups terminate at 3.3 V."], interventionSafety: ["Power down before changing I²C wiring."], reference: { kind: "baseline_characterization", description: "No independent orientation reference is declared." } }, constraints: commonConstraints }),
