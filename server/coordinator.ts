...
export class Coordinator {
  ...
  async report(id: string): Promise<DiagnosisReport> {
    const session = await this.getSession(id);
    return { sessionId: session.id, evidenceSource: session.mode, project: session.projectContext.project, profile: session.projectContext.profile, targetId: session.targetId, reportedProblem: session.problem, observed: session.evidence.observed, inference: session.evidence.inferences, recommendation: session.evidence.recommendations, verification: session.evidence.verification, confidence: session.evidence.confidence, limitations: session.evidence.limitations, experiments: session.observations, intervention: session.intervention, status: session.lifecycle, timing: { machineActiveMs: session.observations.reduce((sum, observation) => sum + Math.max(0, observation.monotonicEndedMs - observation.monotonicStartedMs), 0), wallClockMs: this.now().getTime() - new Date(session.createdAt).getTime() }, agenticProof: session.decisions.some((decision) => decision.decisionSource === "openai") };
  }
